...
export default function RealizarAgendamento() {
  ...
  function validarCampos() {
    ...
    if (servicosSelecionados.length === 0)
      novosErros.servicos = "Selecione pelo menos um serviço.";
    if (!descricaoProblema.trim())
      novosErros.descricao = "Descreva o problema.";
    setErros(novosErros);
  ...
  async function handleConfirmAgendamento() {
    ...
    try {
      ...
      console.groupEnd();
      setPopupSucesso({ show: true, mensagem: "Agendamento criado com sucesso!" });
      handleCancelar();
      setShowResumo(false);
    } catch (err) {
      ...
      console.error(err);
      console.groupEnd();
      setPopupErro({ show: true, mensagem: "Não foi possível criar o agendamento. Tente novamente." });
    } finally {
  ...
  async function handleAvancar() {
    if (!validarCampos()) {
      ...
      setPopupErro({ show: true, mensagem: "Preencha todos os campos obrigatórios." });
      return;
    }
  ...
  useEffect(() => {
    ...
    apiRequest(
      `http://localhost:8080/agendamentos/gerar-horas-disponiveis?data=${data}`
    )
      .then((horarios) => {
  ...
  useEffect(() => {
    ...
    if (!usuarioId) {
      console.warn(
        "usuarioId não encontrado no sessionStorage. O select de veículos ficará vazio."
      );
      setVeiculos([]);
      return;
    }
    ...
    <>
      <div className="min-w-screen min-h-screen bg-[#2B2B2B] flex items-center justify-center overflow-y-auto">
        <div className="flex flex-col items-center gap-10">
          <div className="flex w-[85%] max-h-[90vh]">
            <div className="w-1/2 flex flex-col gap-15">
              ...
              <div className="flex flex-col gap-5">
                <span className="text-2xl">
                  Reserve seu atendimento em poucos cliques!
                </span>
                <div className="w-[82%] flex justify-between">
                  <div className="w-[45%] flex flex-col gap-2">
                    <span className="text-l">
                      Qual veículo precisa de atendimento?
                    </span>
                    <select
                      name="slct-veiculos"
                      className="w-full h-full bottom-0! cursor-pointer rounded-full text-center bg-gray-200 text-orange-500 border-2 border-amber-600"
                  ...
                  <div className="w-[45%] flex flex-col gap-2">
                    ...
                    <input
                      ...
                      className="w-full flex justify-center rounded-full bg-gray-200 text-orange-500 border-2 border-amber-600"
                    />
                  </div>
                </div>
                <div className="w-[82%] flex flex-col gap-2">
                  ...
                  <select
                    ...
                    className="w-full h-[27.2px] rounded-full flex justify-center text-center bg-gray-200 text-orange-500 border-2 border-amber-600"
                  >
                    <option value="" disabled>
                      {isDomingo
                ...
                <div className="w-full flex flex-col gap-2">
                  <span className="w-[82%] text-l">
                    Conte-nos qual é o problema para que possamos te atender da
                    melhor forma.
                  </span>
                  <textarea
                    name="descricao"
                    ...
                    className="w-[82%] px-4! py-3! h-36 resize-none bg-white/25 rounded-md placeholder:text-white"
                ...
                className={`w-70 h-11 rounded-2xl! border-[none]! flex items-center justify-center self-end text-3xl! font-semibold! leading-none! transition-colors
                  ${
                    formCompleto && !isSubmitting
                      ? "bg-green-600! hover:bg-green-700! hover:border-white! hover:border-2! text-white"
      ...
      <ResumoAgendamentoModal
        ...
        dados={{
          ...buildPayload(),
          servicosDetalhes: servicos.filter((s) =>
            servicosSelecionados.includes(s.servicoId)
          ),
        }}
